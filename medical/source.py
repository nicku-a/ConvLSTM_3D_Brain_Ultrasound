...
from devito.types.caching import _SymbolCache

__all__ = ['PointSource', 'Receiver', 'Shot', 'WaveletSource',
           'RickerSource', 'GaborSource', 'ToneBurstSource', 'DGaussSource', 'TimeAxis']
...
class WaveletSource(PointSource):
    ...
    def wavelet(self, f0, t):
        ...
        t : TimeAxis
            Discretized values of time in s.
        """
...
class ToneBurstSource(WaveletSource):
    """
    Symbolic object that encapsulate a set of sources with a
    pre-defined tone-burst wavelet:
    ...
    def wavelet(self, f0, t, n_cycles=3):
        """
        Defines a ToneBurst wavelet with a peak frequency f0 at time t.
        ...
        tone_length = n_cycles / f0
        dt = t[1] - t[0]
        offset = 0
        n_samples = t.shape[0]

        time_array, step = np.linspace(0, tone_length, tone_length / dt + 1, retstep=True, endpoint=False)
        tone_burst = np.sin(2 * np.pi * f0 * time_array)
